function dppLSTM_evaluation( data_dir, dataset, file_name)
...
for i = 1:length(test_inds)
    ...
    [videos_test_scores(tid).Ypred] = gen_summ_key(lstm_pred, lstm_qual, frm_sample, shot_boundary, num_frm.cFrm(tid));
    videos_test(tid).Ypred = get_th(videos_test_scores(tid).Ypred, shot_boundary);
end
save videos_test_scores.mat videos_test_scores
